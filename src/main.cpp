...
void bouncing_spheres() {
  ...
  auto checker =
      make_shared<checker_texture>(0.32, color(.0, .0, .0), color(.9, .9, .9));
...
void final_scene(int image_width, int samples_per_pixel, int max_depth) {
  ...
  auto ground = make_shared<metal>(color(0.48, 0.83, 0.53), 0.2);
  ...
  int ns = 1000;
  for (int j = 0; j < ns; j++) {
    boxes2.add(make_shared<sphere>(point3::random(0, 165), 10, ground));
  }
